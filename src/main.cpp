...
#include "core_engine/io/sound.hpp"
#include "core_engine/component/2d/text/text_component.hpp"
#include "core_engine/component/particle/particle_system.hpp"
#include "core_engine/component/components.hpp"
#include "core_engine/component/movement.hpp"
#include "core_engine/component/mesh/advanced_mesh.hpp"
#include "core_engine/component/mesh/animated_mesh.hpp"
#include "core_engine/component/test/chaser.hpp"
#include "core_engine/component/2d/gui.hpp"
#include "core_engine/component/2d/chat_box.hpp"
#include "rendering_engine/model/obj_loader.hpp"
#include "rendering_engine/shader/material.hpp"

int main(int argc, char *argv[])
{
    window* main_window = new window(824, 600, "Sonni's 3D Game Engine");
    ...
    entity* character = _core_engine->create_mesh_phong("mesh/animation.glsl", vec3f(30, 45, 40), 1, &Phong_light, new animation_component("running_man", &shadow_mvp, &depth_map, p_player,  "", ""));
    ...
    entity* terrain1 = _core_engine->create_terrain("terrain/terrain_normal_map.glsl", vec3f(0, 0, 0), 1, &Phong_light, new terrain_component(0, 0, 512, "terrain/grass.png", "terrain/grass.png", "terrain/grass.png", "terrain/road.png", "terrain/blendmap.png", &shadow_mvp, &depth_map), &terrains);
    ...
    entity* game_text = _core_engine->create_mesh("2d/text.glsl", vec3f(), 1, new text_component("3D Game Engine By Sonni", "arial", 3.0f, vec2f(0.0f, 0.0f), 1.0f, true));
    entity* health_bar = _core_engine->create_mesh("2d/gui.glsl", vec3f(150, main_window->get_height() - 150, 0), 0.3f, new gui_component("gui/healthBar.png", hud_mesh, main_window->get_width(), main_window->get_height()));
    entity* monkey2 = _core_engine->create_mesh_phong("mesh/simpel_mesh.glsl", vec3f(60, 30, 60), 7, &Phong_light, new advanced_mesh(new mesh("monkey"), &shadow_mvp, &depth_map, vec3f(0.3, 0.3, 0.3), mk2, "bricks.jpg"));
    ...
    entity* phong_mesh = _core_engine->create_mesh_phong("mesh/advanced_mesh.glsl", vec3f(30, 10, 30), 15, &Phong_light, new advanced_mesh(new mesh("plane"), &shadow_mvp, &depth_map, vec3f(0.3, 0.3, 0.3), new physics_obj(new aabb(vec3f(0, 0, 0), vec3f(10.0, 10.0, 10.0))), "bricks.jpg", "bricks_normal.jpg", "bricks_disp.jpg"));

    entity* chaser = _core_engine->create_mesh_phong("mesh/advanced_mesh.glsl", vec3f(50, -1, 50), 3, &Phong_light, new advanced_mesh(new mesh("monkey"), &shadow_mvp, &depth_map, vec3f(0.3, 0.3, 0.3), mk2, "lantern.png", "bricks_normal.jpg", "default_disp.png", "lantern.png"));
    ...
    entity* chat_box = new entity(new shader("2d/mesh.glsl"), vec3f(100, 100, 0));
    chat_box->add_component(new chat_box_component(100, 100, main_window));

    obj_loader lantern_mesh("lantern.obj");

    entity* lantern = _core_engine->create_mesh_phong("mesh/advanced_mesh.glsl", vec3f(130, -3, 60), 2, &Phong_light, new advanced_mesh(lantern_mesh.gen_mesh(), &shadow_mvp, &depth_map, vec3f(0.1, 0.1, 0.1), mk2, "lantern.png", "default_normal.jpg", "default_disp.png", "lanternS.png"));
    lantern->get_transform()->rotate(vec3f(1,0,0), -ToRadians(90));
    ...
    std::cout << m.get_int("int1") << std::endl;
    ...
    _core_engine->add_entity(lantern);
   // _core_engine->add_entity(chat_box);
    ...
    _core_engine->add_scatter(terrain1);
    _core_engine->add_scatter(monkey2);
    _core_engine->add_scatter(lantern);
