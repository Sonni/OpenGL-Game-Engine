...
void main()
{
    ...
    vec2 tex_coords = (tex_coord.xy + (normalize(to_cam_vec)).xy * (texture(disp_tex, tex_coord.xy).r * 0.02 + 0.0));
    ...
    vec4 normal_tex_value = 2.0 * texture(normal_tex, tex_coords) - 1.0;

	vec3 unit_normal = normalize(normal_tex_value.rgb);
	...
	vec4 texture_color = texture(model_tex, tex_coords);
