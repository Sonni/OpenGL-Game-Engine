...
mat4f camera::get_view_projection() const
{
    ...
    return m_projection_matrix * cam_conj_rot_matrix * cam_translation_matrix;
}

mat4f camera::get_view_matrix() const
{
    mat4f cam_rot_matrix;
    cam_rot_matrix.init_rotation_from_direction(m_transformation->get_rot()->get_forward(), m_transformation->get_rot()->get_up());

    mat4f cam_translation_matrix;
    cam_translation_matrix.init_translation(*m_transformation->get_pos());
...
{
    float new_pitch = -ToDegrees(asinf(m_transformation->get_rot()->get_forward().get_y()));
    float cur_pitch = ToDegrees(asinf(m_transformation->get_rot()->get_forward().get_y()));
    float amount = new_pitch - cur_pitch;

    quaternion tmp_pitch = quaternion(vec3f(1, 0, 0), ToRadians(-amount));

    m_transformation->set_rot(old_rot * tmp_pitch);
    return old_rot;
}
